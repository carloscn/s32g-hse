...
void SystemCoreClockUpdate(void)
{
    ...
    switch ((MC_CGM_0->MUX_0_CSS & MC_CGM_MUX_0_CSS_SELSTAT_MASK) >> MC_CGM_MUX_0_CSS_SELSTAT_SHIFT)
    {
        case 0x0U:
            /* 48MHz int. RC osc */
        ...
        case 0xCU:
            ...
            if ((CORE_PLL->PLLCLKMUX & CORE_PLL_PLLCLKMUX_REFCLKSEL_MASK) == 0U)
            {
            ...
            prediv = ((CORE_PLL->PLLDV & CORE_PLL_PLLDV_RDIV_MASK) >> CORE_PLL_PLLDV_RDIV_SHIFT);
            if (prediv == 0x0U)
            {
                prediv = 1U;
            }
            ...
            multi = ((CORE_PLL->PLLDV & CORE_PLL_PLLDV_MFI_MASK) >> CORE_PLL_PLLDV_MFI_SHIFT);
            ...
            if ((CORE_PLL->PLLFD & CORE_PLL_PLLFD_SDMEN_MASK) != 0U)
            {
                /* Calculate numerator for fractional loop division factor */
                numerator = ((CORE_PLL->PLLFD & CORE_PLL_PLLFD_MFN_MASK) >> CORE_PLL_PLLFD_MFN_SHIFT);
